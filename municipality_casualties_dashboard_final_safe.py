...
@st.cache_data
def load_data():
    return pd.read_excel("total_casualties_with_coordinates.xlsx")
...
lang_dict = {
    ...
    "map_title": {
        "en": "Map View (All municipalities with coordinates)",
        "he": "תצוגת מפה (כל הרשויות עם קואורדינטות)"
    }
}
...
df_map = df.dropna(subset=["Latitude_y", "Longitude_y", metric])
df_map[metric] = pd.to_numeric(df_map[metric], errors="coerce")
